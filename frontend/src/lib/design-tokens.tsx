  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let y = 0; y < ch; y += sp) {
        ctx.beginPath();
        ctx.moveTo(0, y);
        for (let x = 0; x < cw; x += 4) ctx.lineTo(x, y + Math.sin(x * 0.015 + y * 0.01) * 2.5);
      ...
      for (let x = 0; x < cw; x += sp * 2) {
        ctx.beginPath();
        ctx.moveTo(x, 0);
        for (let y = 0; y < ch; y += 4) ctx.lineTo(x + Math.sin(y * 0.012) * 2.5, y);
      ...
      {colors.map((c, i) => (
        <div
          ...
          style={{
            width: size,
            height: size,
            borderRadius: Math.max(1.5, size * 0.14),
            background: c,
...
export function MasteryCell({ value, label }: { value: number; label: string }) {
