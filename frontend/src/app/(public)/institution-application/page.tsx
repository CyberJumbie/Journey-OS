...
export default function InstitutionApplication() {
  ...
  // Input component
  const Input = ({ label, name, type = "text", placeholder, required, value, error }: { label: string; name: string; type?: string; placeholder?: string; required?: boolean; value: string; error?: string }) => (
  ...
  // Select component
  const Select = ({ label, name, options, value, error }: { label: string; name: string; options: { value: string; label: string }[]; value: string; error?: string }) => (
    <div style={{ marginBottom: 20 }}>
      <label htmlFor={name} style={{
        display: "block",
        fontFamily: mono,
