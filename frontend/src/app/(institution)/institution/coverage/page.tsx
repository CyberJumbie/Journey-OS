...
import { BarChart3, Users, Settings, Shield, Activity, Award, TrendingUp, AlertCircle, GitBranch, GraduationCap, Layers } from "lucide-react";
import { C, sans, serif, mono } from '@/lib/design-tokens';
import { useBreakpoint } from '@/hooks/useBreakpoint';
import { useInstitutionCoverage } from '@/hooks/useDashboard';
...
export default function CoverageDashboard() {
  ...
  const { data: apiCoverageItems } = useInstitutionCoverage();

  // Map API CoverageItem[] to local display format
  const coverageData: CoverageDisplayItem[] = (apiCoverageItems ?? []).map((item, i) => {
    const getPriority = (count: number): "high" | "medium" | "low" => {
    ...
    return {
      id: String(i + 1),
      domain: item.category_type === "usmle_system" ? "USMLE Systems" : "USMLE Disciplines",
      subdomain: item.category,
      coverage_percent: 0, // coverage percentage not in API; item_count drives display
      question_count: item.item_count,
      gap_priority: getPriority(item.item_count),
    };
  });
  ...
  const getPriorityColor = (priority: CoverageDisplayItem["gap_priority"]) => {
    switch (priority) {
  ...
  const groupedData = coverageData.reduce((acc, item) => {
    ...
    return acc;
  }, {} as Record<string, CoverageDisplayItem[]>);
