...
import { BarChart3, Users, Settings, Shield, Activity, Award, ChevronRight, GitBranch, GraduationCap, Layers } from "lucide-react";
...
export default function FrameworkConfiguration() {
  ...
  const handleToggleFramework = async (id: string, currentState: boolean) => {
    try {
      setFrameworks((prev: Framework[]) =>
        prev.map((f: Framework) => (f.id === id ? { ...f, enabled: !currentState } : f))
      );
        ...
        <div style={{ padding: isMobile ? "20px 16px" : isTablet ? "24px 24px" : "28px 32px", maxWidth: 1400 }}>
          ...
          <div style={{
            ...
            {[
              ...
              { label: "Enabled", value: frameworks.filter((f: Framework) => f.enabled).length },
              { label: "Avg Coverage", value: `${Math.round(frameworks.filter((f: Framework) => f.enabled).reduce((sum: number, f: Framework) => sum + f.coverage_percent, 0) / frameworks.filter((f: Framework) => f.enabled).length || 0)}%` },
            ].map((stat) => (
                    ...
                    <div key={framework.id} style={{
                      ...
                      {framework.enabled && (
                        <div style={{
                          ...
                          <button
                            onClick={() => router.push(`/institution/frameworks/${framework.id}`)}
                            style={{
                              padding: "8px 16px",
                              background: C.white,
