...
const PIPELINE_STATUS = { IDLE: "idle", RUNNING: "running", COMPLETED: "completed", FAILED: "failed" } as const;
...
function ChatPanel({ messages, mode, selectedCourse, pipelineStatus }: { messages: { role: string; text: string }[]; mode: string; selectedCourse: string | null; pipelineStatus: string }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {messages.length === 0 && pipelineStatus === PIPELINE_STATUS.IDLE ? (
          <div style={{
            display: "flex",
            flexDirection: "column",
            alignItems: "center",
            ...
            <MessageSquare size={32} style={{ color: C.textMuted, marginBottom: 16, opacity: 0.4 }} />
            <p style={{
              fontFamily: sans,
              fontSize: 14,
              color: C.textMuted,
              ...
              Start a conversation to generate questions. Try one of these:
            </p>
            <div style={{ display: "flex", flexDirection: "column", gap: 8, width: "100%", maxWidth: 320 }}>
              {[
                "Generate a clinical vignette on ACS pathogenesis",
                "Create 5 items covering cardiovascular pharmacology",
                "Help me fill coverage gaps in my syllabus",
              ].map((prompt, i) => (
                <div key={i} style={{
                  padding: "10px 14px",
                  borderRadius: 8,
      ...
      <div style={{
        ...
        <div style={{
          ...
          <input
            type="text"
            placeholder={!selectedCourse ? "Select a course first..." : mode === MODES.REVIEW ? "Suggest changes to this question..." : "Guide the generation..."}
          ...
          <button
            disabled={!selectedCourse}
            title={!selectedCourse ? "Select a course first" : undefined}
            style={{
              background: selectedCourse ? C.green : C.textMuted,
              border: "none",
              borderRadius: 6,
              padding: "8px 16px",
              color: C.white,
...
function OptionRow({ opt, showMisconception }: { opt: { letter: string; text: string; correct: boolean; misconception: string | null }; showMisconception: boolean }) {
  return (
    <div style={{
      padding: "12px 16px",
      marginBottom: 8,
...
interface QuestionData {
  ...
  proficiencyVariable: string;
  taskShell: string;
  toulmin: { claim: string; warrant: string; backing: string; rebuttal: string };
  sources: QuestionSource[];
}

function QuestionPreview({ question, showRationale }: { question: QuestionData; showRationale: boolean }) {
  return (
      ...
      {showRationale && (
        <>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            {question.sources.map((s, i: number) => (
              <div key={i} style={{
                display: "flex",
                alignItems: "center",
                justifyContent: "space-between",
...
export default function QuestWorkbench() {
  ...
  const [selectedCourse] = useState<string | null>("MEDI-531 Cardiovascular System");
  const [pipelineStatus] = useState(PIPELINE_STATUS.IDLE);
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <ChatPanel messages={messages} mode={mode} selectedCourse={selectedCourse} pipelineStatus={pipelineStatus} />
        </div>
