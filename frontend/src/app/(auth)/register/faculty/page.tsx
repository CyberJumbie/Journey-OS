...
const DEPARTMENTS = [
  "Anatomy", "Biochemistry", "Cardiology", "Emergency Medicine",
  "Family Medicine", "Internal Medicine", "Neurology",
  "Obstetrics and Gynecology", "Pathology", "Pediatrics",
  "Pharmacology", "Physiology", "Psychiatry", "Radiology", "Surgery",
];
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div>
              ...
              <select
                ...
                onFocus={e => { if (!errors.department) { e.target.style.borderColor = C.blueMid; e.target.style.boxShadow = `0 0 0 3px ${C.blueMid}15`; } }}
                onBlur={e => { e.target.style.borderColor = errors.department ? C.error : C.border; e.target.style.boxShadow = "none"; }}
              >
                <option value="">Select department...</option>
                {DEPARTMENTS.map(dept => <option key={dept} value={dept}>{dept}</option>)}
              </select>
              {errors.department && <p style={{ fontSize: 12, color: C.error, marginTop: 4 }}>{errors.department}</p>}
            </div>
