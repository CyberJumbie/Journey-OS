...
- Run `tsc --noEmit` and `vitest run` from within the workspace directory (`cd apps/server`, `cd apps/web`), not from monorepo root. Root-level `npx tsc` picks up a global tsconfig, and `npx vitest` with absolute paths may resolve to the wrong workspace. **Exact tsc commands** (no `npx`, no `pnpm exec` — use the workspace-local binary directly): `packages/types/node_modules/.bin/tsc -b packages/types/tsconfig.json` for types rebuild; `apps/server/node_modules/.bin/tsc --noEmit` for server type-check (run from monorepo root with these paths).
...
- **CRITICAL (recurrence: 2)** — PostToolUse eslint hook strips "unused" exports/imports from `index.ts` barrel files. TWO mandatory steps: (1) After editing any barrel file, IMMEDIATELY re-read it and verify exports are intact. Re-add if stripped. (2) When adding imports AND their usage (e.g., route registration), add BOTH in a single Edit call — never split across two edits.
