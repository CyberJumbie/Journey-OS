...
export function LoginScreen() {
  ...
  return (
    <AuthLayout
      headline="Every thread of your curriculum, woven together."
      subheadline="AI-powered assessment generation, curriculum mapping, and student mastery tracking — all connected in one knowledge graph."
    >
      {/* Header */}
      ...
      {callbackError && (
        <div
          className="mb-4 rounded-lg text-sm text-error"
          style={{
            padding: "10px 14px",
        ...
        {roles.map((r) => (
          <button
            ...
            style={{
              ...
              color:
                activeRole === r.key ? "var(--navy-deep)" : "var(--text-muted)",
              background: activeRole === r.key ? "var(--white)" : "transparent",
              border: "none",
              borderRadius: 6,
              padding: "9px 0",
              cursor: "pointer",
              transition: "all 0.2s",
              boxShadow: activeRole === r.key ? "var(--shadow-subtle)" : "none",
            }}
          >
            {r.label}
          </button>
        ...
        <div className="flex items-center justify-between">
          <label className="flex cursor-pointer items-center gap-2">
            <div
              ...
              tabIndex={0}
              onKeyDown={(e) => {
                if (e.key === " " || e.key === "Enter") setRemember(!remember);
              }}
              style={{
                width: 18,
                height: 18,
